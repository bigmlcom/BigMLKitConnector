...
internal class PredictionTree {
    ...
    required init(tree : [String : AnyObject],
        ...
        inout treeInfo : [String : AnyObject]) {
            ...
            if let nodes : [[String : AnyObject]] = tree["children"] as? [[String : AnyObject]] {
            ...
            if let distributionObject = tree["distribution"] as? [(value : AnyObject, dist : Int)] {
                ...
                if let bins = summary["bins"] as? [(value : AnyObject, dist : Int)] {
                    self.distribution = bins
                    self.distributionUnit = "bins"
                } else if let counts = summary["counts"] as? [(value : AnyObject, dist : Int)] {
            ...
            if (self.regression) {
                ...
                self.maximum = summary["maximum"] as? Double ??
                    self.distribution.map { $0.value as? Double ?? Double.NaN  }.maxElement() ?? Double.NaN
                self.minimum = summary["minimum"] as? Double ??
                    self.distribution.map { $0.value as? Double ?? Double.NaN }.minElement() ?? Double.NaN
            }
    ...
    */
    func predictProportional(arguments : [String : AnyObject],
        ...
        missingFound : Bool,
        median : Bool)
        -> (distribution : [(value : AnyObject, dist : Int)], minimum : Double, maximum : Double, lastNode : PredictionTree) {
    ...
    */
    func predict(arguments : [String : AnyObject],
        ...
        -> (prediction: TreePrediction, path: [String]) {
            ...
            var p = path
            if strategy == MissingStrategy.LastPrediction {
                for child in self.children {
                    if child.predicate.apply(arguments, fields: self.fields) {
                ...
                return (prediction: TreePrediction(prediction: self.output,
                    ...
                    children: self.children), path: path)
                ...
                if self.regression {
                    ...
                    var distribution = finalDistribution
                    ...
                    let confidence = regressionError(
                        ...
                        instances: totalInstances, rz: DEFAULT_RZ)
                    
                    let p = TreePrediction(prediction: m,
                    ...
                    return (prediction: p, path: path)
                }
            }
            assert(false, "Unsupported missing strategy \(strategy)")
            return (TreePrediction(prediction: 0.0,
