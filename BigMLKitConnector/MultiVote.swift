...
class MultiVote {
    ...
    */
    required init(predictions : [[String : Any]]) {
        ...
        //        let ordered = !self.predictions %%{ (prediction)
        //            prediction["order"] == nil
        //        }
        var ordered = true
    ...
    */
    func normalizedError(range : Double, topRange : Double, rangeMin : Double) -> Double {
        ...
        if range > 0.0 {
            for var p in self.predictions {
                let delta = rangeMin - (p["confidence"] as? Double ?? Double.NaN)
                p.updateValue(exp(delta / range * topRange), forKey: "errorWeight")
                normalizedError += (p["errorWeight"] as? Double ?? Double.NaN)
            }
        } else {
            for var p in self.predictions {
                p.updateValue(1.0, forKey: "errorWeight")
            }
    ...
    */
    func weightedError(confidence : Bool,
        ...
        addMax : Bool) -> [String : Any] {
            ...
            var newPrediction : [String : AnyObject] = [:]
            ...
            for p in self.predictions {
                ...
                let medianError = (p["median"] as? Double ?? Double.NaN) *
    ...
    func weightedConfidence(combinedPrediction : AnyObject,
        weightLabel : String) -> [String : Any] {
            ...
            let predictions = self.predictions.filter{
                ...
                } else if let predictio = $0["prediction"] as? Double {
                    return predictio == (combinedPrediction as? Double ?? Double.NaN)
    ...
    */
    func combineCategorical(weightLabel : String, confidence : Bool) -> [String : Any] {
        ...
        for p in self.predictions {
            ...
            var categoryHash : [String : Any] = [:]
            if mode[category] != nil {
                categoryHash.updateValue(mode[category]?["count"] as? Double ?? Double.NaN,
                    forKey: "count")
                categoryHash.updateValue(mode[category]?["order"] as? Double ?? Double.NaN,
                    forKey: "order")
            } else {
                categoryHash.updateValue(weight, forKey: "count")
                categoryHash.updateValue(p["order"] ?? 0, forKey: "order")
            }
...
//            
    ...
    */
    func combine(method : PredictionMethod,
        ...
        addMin : Bool,
        addMax : Bool,
        options : [String : AnyObject]) -> [String : Any] {
